...
internal interface RetryStrategy {
     ...
     * 
     * This method should be called each time a retry attempt fails to gradually increase
     ...
     * 
     * This method should be called when a retry attempt succeeds to reset the backoff strategy
     * for future retry attempts.
     */
     ...
     * randomization (jitter). This randomization helps avoid overwhelming the backend by
     * preventing all clients from retrying at exactly the same time.
     *
 ...
 * 
 * This implementation uses an exponential backoff strategy with randomization to distribute
 * retry attempts over time. The delay increases with each consecutive failure up to a maximum
 * limit to balance between quick recovery and avoiding server overload.
 */
internal class DefaultRetryStrategy : RetryStrategy {
    ...
    companion object {
         ...
         * 
         * This prevents the retry delay from growing indefinitely and ensures that
         * reconnection attempts don't become excessively infrequent.
         */
     ...
     * 
     * This increases the delay for subsequent retry attempts according to the
     ...
     * 
     * This should be called when a connection succeeds to reset the retry delay
     * for future attempts.
     */
