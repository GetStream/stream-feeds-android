 ...
 *
 * This class manages the reconnection attempts based on various policies and strategies.
 * It listens to lifecycle events and network state changes to determine when to reconnect or
 * disconnect.
 *
...
internal class ConnectionRecoveryHandler(
    ...
    private val automaticReconnectionPolicies: List<AutomaticReconnectionPolicy> = listOf(
    ...
    private val lifecycleListener = object : StreamLifecycleObserver.LifecycleListener {
        ...
        override suspend fun onStop() {
            ...
            if (!canDisconnect) {
                logger.d { "[onStop] Socket is not connecting/connected (${socket.connectionState})" }
                return
            }
    ...
    private val networkStateListener = object : NetworkStateProvider.NetworkStateListener {
        ...
        override suspend fun onDisconnected() {
            logger.d { "[odDisconnected] Network not available. Ignoring this event as the " +
                    "connection interruption would close the socket." }
        }
    }

    private val stateListener = object : FeedsSocketListener {
    ...
    private val canDisconnect: Boolean
        get() = when (socket.connectionState) {
            is WebSocketConnectionState.Connecting,
    ...
    /**
     * Starts the connection recovery handler by subscribing to the necessary listeners.
     * This method should be called when the socket is initialized.
     */
    ...
    /**
     * Stops the connection recovery handler by unsubscribing from the listeners.
     * This method should be called when the socket is no longer needed.
     */
