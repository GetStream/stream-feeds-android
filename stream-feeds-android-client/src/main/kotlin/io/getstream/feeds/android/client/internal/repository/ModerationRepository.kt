...
internal interface ModerationRepository {
     ...
     *
     * This method fetches moderation configurations from the server using the specified query
     * parameters, including filtering, sorting, and pagination options.
     *
     ...
     *   parameters.
     * @return A [Result] containing a [BanResponse] if the ban is successful, or an error if the
     *   ban fails.
     */
     ...
     * @return A [Result] containing a [MuteResponse] if the mute operation is successful, or an
     ...
     * @return A [Result] containing a [BlockUsersResponse] if the blocking is successful, or an
     *   error if the operation fails.
     */
     ...
     * @return A [Result] containing a [UnblockUsersResponse] if the unblocking is successful, or an
     *   error if the operation fails.
     */
     ...
     * @return A [Result] containing a [FlagResponse] if the flagging is successful, or an error if
     *   the operation fails.
     */
     ...
     * @return A [Result] containing an [UpsertConfigResponse] if the upsert is successful, or an
     ...
     */
    suspend fun getConfig(key: String, team: String? = null): Result<GetConfigResponse>
     ...
     *   parameters.
     * @return A [Result] containing a [QueryModerationConfigsResponse] with the list of moderation
     *   configurations, or an error if the query fails.
     */
    suspend fun queryModerationConfigs(
