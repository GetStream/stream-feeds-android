 ...
 * related to poll updates. It maintains an internal state that reflects the current
 * list of polls and their pagination status.
 *
...
internal class PollListImpl(
    ...
    private suspend fun queryPolls(query: PollsQuery): Result<List<PollData>> {
        return pollsRepository.queryPolls(query)
            .onSuccess {
                _state.onQueryMorePolls(it, QueryConfiguration(query.filter, query.sort))
            }
            .map {
                it.models
            }
