...
public interface ActivityList {
     ...
     *
     * This method retrieves the first page of activities using the filtering and sorting parameters
     * defined in the query. The results are automatically stored in the state and can be observed
     * through the [state.activities] property.
     *
     * @return A [Result] containing a list of [ActivityData] if the fetch is successful, or an
     ...
     *
     * This method retrieves additional activities using the pagination cursor from the previous
     * request. The new activities are automatically merged with the existing activities in the
     * state, maintaining the proper sort order.
     *
     * @param limit Optional limit for the number of activities to fetch. If not specified, the
     *   default limit from the query will be used.
     * @return A [Result] containing a list of [ActivityData] if the fetch is successful, or an
     *   error if the fetch fails. Returns an empty list if there are no more activities to fetch.
     */
