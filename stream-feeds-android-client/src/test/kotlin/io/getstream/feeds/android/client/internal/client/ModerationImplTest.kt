...
internal class ModerationImplTest {
    ...
    fun `blockUser when given request, then delegates to repository`() = runTest {
        ...
        coEvery { moderationRepository.blockUser(blockRequest) } returns Result.success(blockResponse)
    ...
    fun `unblockUser when given request, then delegates to repository`() = runTest {
        ...
        coEvery { moderationRepository.unblockUser(unblockRequest) } returns Result.success(unblockResponse)
    ...
    fun `unblockUser when repository fails, then returns failure`() = runTest {
        val unblockRequest = UnblockUsersRequest(blockedUserId = "user-123")
        val exception = RuntimeException("Network error")
        coEvery { moderationRepository.unblockUser(unblockRequest) } returns Result.failure(exception)
    ...
    fun `getBlockedUsers when called, then delegates to repository`() = runTest {
        ...
        coEvery { moderationRepository.getBlockedUsers() } returns Result.success(blockedUsersResponse)
    ...
    fun `submitAction when given request, then delegates to repository`() = runTest {
        ...
        coEvery { moderationRepository.submitAction(submitActionRequest) } returns Result.success(submitActionResponse)
    ...
    fun `submitAction when repository fails, then returns failure`() = runTest {
        ...
        val exception = RuntimeException("Network error")
        coEvery { moderationRepository.submitAction(submitActionRequest) } returns Result.failure(exception)
    ...
    fun `queryReviewQueue when given request, then delegates to repository`() = runTest {
        ...
        coEvery { moderationRepository.queryReviewQueue(queryRequest) } returns Result.success(queryResponse)
    ...
    fun `queryReviewQueue when repository fails, then returns failure`() = runTest {
        val queryRequest = QueryReviewQueueRequest()
        val exception = RuntimeException("Network error")
        coEvery { moderationRepository.queryReviewQueue(queryRequest) } returns Result.failure(exception)
    ...
    fun `upsertConfig when given request, then delegates to repository`() = runTest {
        ...
        coEvery { moderationRepository.upsertConfig(upsertRequest) } returns Result.success(upsertResponse)
    ...
    fun `upsertConfig when repository fails, then returns failure`() = runTest {
        val upsertRequest = UpsertConfigRequest("key")
        val exception = RuntimeException("Network error")
        coEvery { moderationRepository.upsertConfig(upsertRequest) } returns Result.failure(exception)
    ...
    fun `deleteConfig when given key and team, then delegates to repository`() = runTest {
        ...
        coEvery { moderationRepository.deleteConfig(key, team) } returns Result.success(deleteResponse)
    ...
    fun `queryModerationConfigs when given request, then delegates to repository`() = runTest {
        ...
        coEvery { moderationRepository.queryModerationConfigs(queryRequest) } returns Result.success(queryResponse)
    ...
    fun `queryModerationConfigs when repository fails, then returns failure`() = runTest {
        val queryRequest = QueryModerationConfigsRequest()
        val exception = RuntimeException("Network error")
        coEvery { moderationRepository.queryModerationConfigs(queryRequest) } returns Result.failure(exception)
