...
internal class FeedEventHandlerTest {
    ...
    @Test
    fun `on ActivityReactionUpserted, then handle based on feed match`() {
        val activity = activityData()
        val reaction = feedsReactionData("activity-1")
        val matchingEvent =
            StateUpdateEvent.ActivityReactionUpserted(fid.rawValue, activity, reaction)
        val nonMatchingEvent =
            StateUpdateEvent.ActivityReactionUpserted("group:different", activity, reaction)
    ...
    @Test
    fun `on BookmarkUpserted, then handle based on activity feed match`() {
