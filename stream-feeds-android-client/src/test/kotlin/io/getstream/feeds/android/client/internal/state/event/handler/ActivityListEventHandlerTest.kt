...
internal class ActivityListEventHandlerTest {
    ...
    @Test
    fun `on ActivityReactionUpserted, then call onReactionUpserted`() {
        val activity = activityData("activity-1")
        val reaction = feedsReactionData("activity-1")
        val event = StateUpdateEvent.ActivityReactionUpserted("feed-1", activity, reaction)
    ...
    @Test
    fun `on BookmarkUpserted, then call onBookmarkUpserted`() {
        val bookmark = bookmarkData()
        val event = StateUpdateEvent.BookmarkUpserted(bookmark)
