...
class LoginManager @Inject constructor(
    ...
    suspend fun currentState(): UserState? = mutex.withLock {
        if (state != null) state
        else {
            loadCredentials()
                ?.let { connect(it) }
    ...
    suspend fun login(credentials: UserCredentials): Result<UserState> = mutex.withLock {
        connect(credentials)
            .onSuccess {
                state = it
    ...
    private suspend fun loadCredentials(): UserCredentials? =
        dataStore.data.first()[loggedUserIdKey]
            ?.let { userId -> UserCredentials.BuiltIn.find { it -> it.user.id == userId } }
    ...
    private suspend fun connect(credentials: UserCredentials): Result<UserState> {
        ...
        return client.connect()
            .map { UserState(user = credentials.user, client = client) }
    }
