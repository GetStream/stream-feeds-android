...
/**
 * Represents the source that initiated a WebSocket disconnection.
 * This helps determine the appropriate reconnection behavior and error handling.
 */
public sealed interface DisconnectionSource {

    /**
     * Disconnection was initiated by the user or client application.
     * This typically means no automatic reconnection should be attempted.
     */
     ...
     * 
     * @param error The API error that caused the disconnection, if any.
     *              This can be used to determine if reconnection should be attempted.
     */
    public class ServerInitiated(public val error: APIError?) : DisconnectionSource

    /**
     * Disconnection was initiated by the system (e.g., network changes, app backgrounding).
     * This typically allows for automatic reconnection attempts.
     */
    public object SystemInitiated : DisconnectionSource

    /**
     * Disconnection occurred because no pong response was received from the server.
     * This indicates a potential network issue and typically allows for reconnection.
     */
 ...
 * This sealed interface provides a type-safe way to handle different connection states
 * and their associated data.
 */
public sealed interface WebSocketConnectionState {
    /**
     * The initial state meaning that there was no attempt to connect yet.
     * This is the starting state before any connection attempts are made.
     */
    public object Initialized : WebSocketConnectionState

    /**
     * The WebSocket is currently attempting to establish a connection.
     * This is a transitional state between initialized/disconnected and connected.
     */
    public object Connecting : WebSocketConnectionState

    /**
     * The WebSocket connection is established and the client is authenticating.
     * This is a transitional state during the authentication process.
     */
     ...
     */
    public data class Disconnecting(public val source: DisconnectionSource): WebSocketConnectionState

    /**
     * The WebSocket is disconnected.
     * 
     * @param source The source that caused the disconnection.
     *               This is used to determine reconnection behavior.
     */
    public data class Disconnected(public val source: DisconnectionSource): WebSocketConnectionState
    ...
    /**
     * Checks if the WebSocket connection is active (not disconnected).
     * An active connection includes connecting, authenticating, connected, and disconnecting states.
     * 
    ...
    public val isAutomaticReconnectionEnabled: Boolean
        get() = when (this) {
            is Disconnected -> when (this.source) {
                is DisconnectionSource.ServerInitiated -> when {
                    // Do not reconnect on token invalid errors or client errors
                    this.source.error?.code == 1000 -> false // TODO: Define a constant for 1000
